...
investor report: `Factor_Study_Landed_Report.html` (in the gitignored `reports/` and the
Drive library `G:\My Drive RES\REsearch_Reports`; regenerate via
`deliverables/build_factor_report.py`). Companion skills:
...
- Street file first: harvest the Street-scope PP panel + transaction table
  (`research/tools/build_factor_panel.py` parsers; captures via `read-investment-suite`).
...
- Friction: BSD top tiers + agent + SSD 4y 16/12/8/4 (exact math:
  `researcher/tax.py`) — alpha only at purchase.
...
## Step 5 — verify & ship
1. Every load-bearing number Tier-1 (Investment Suite caveats / URA / OneMap /
   SLA — official or app-caveat data only; agent/portal research is low-trust,
   asking prices are never transactions). Full rule:
   `.claude/memory/data-source-trust-hierarchy.md`.
