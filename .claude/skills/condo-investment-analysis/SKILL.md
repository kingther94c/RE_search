...
investor report: `Factor_Study_Condo_Report.html`.
...
Run the deterministic pipeline (skill: `value-a-property`): three-surface comps,
fitted trend + floor premium, triangulation band. Anchor = floor-adjusted freshest
...
## Step 2 — priced-in factor checklist 【实测: direction stable under
leave-one-anchor-out; magnitudes NOT reliable — n=26, anchor-clustered】
Within-segment psf premium ordering: **popular-primary-1km > newness > MRT
proximity ≈ freehold**; project size unstable (dropped). Rules:
...
- At resale, refuse to pay the newness premium (it is developer margin, not a
  durable factor)【牛方实操】.
- Yield: segment choice, not unit-picking. CCR ~2.1–2.5% < RCR < OCR ~3–4.2%
  【实测】; within-segment differences are UNDETECTED at this sample size (power:
