...
<div class='box'>价格/租金比（非有地；价格指数÷租金指数，两者均以 2009Q1=100，故比率在 2009Q1=100；
序列起点 1998Q4 为 99.7）：现 <b>{pr['last'][1]}</b> vs 峰值 <b>{pr['peak'][1]}（{pr['peak'][0]}）</b>
——『部分正常化』：2021 后租金追上价格（2022-23 移民潮+完工荒有一次性成分），但仍比 1998/2009 两个基线贵 ~30%，
...
下表为各视角裁定与最终保留口径。完整意见见 researcher/factors/dialectic_synthesis.json。
C5 的街道证据部分属 Landed 篇（Factor_Study_Landed_Report），此处一并列出以保完整。</p>
...
def build_condo() -> str:
    ...
    # panel-derived yield bands (a reviewer caught stylized bands presented as
    # panel facts — always derive display bands from the shipped rows)
    import statistics as _st
    yb = []
    for seg in ("CCR", "RCR", "OCR"):
        ys = sorted(r["yield_avg"] for r in rows if r["segment"] == seg and r.get("yield_avg"))
        if ys:
            yb.append(f"{seg} 中位 {_st.median(ys):.2f}%（{min(ys):.2f}–{max(ys):.2f}%，n={len(ys)}）")
    yield_bands = " · ".join(yb)
    panel_rows = "".join(
    ...
    stable = [ZH.get(f, f) for f, v in loo.items() if v.get("sign_stable")]
    unstable = [ZH.get(f, f) for f, v in loo.items() if not v.get("sign_stable")]
    loo_summary = ("、".join(stable) + " 方向全部稳定"
                   + (f"；{'、'.join(unstable)} 不稳定（剔除）" if unstable else "（无剔除项）"))
    ols_rows = "".join(
...
②段内溢价最强的两个方向是 <b>楼龄（新）与热门小学 1km</b>，其余（盘规模/MRT/海岸/权属）弱但方向稳定
（全部 5 因子逐锚剔除方向稳定；bootstrap 星号仅学校与权属保留——量级一律不作卖点，adj R²={o.get('adj_r2', 0):.2f}），
这些是<b>已定价因子</b>：按公允价买入不产生 α，其价值在持有体验与下行韧性；
③收益率主要是段位选择（CCR 低 &lt; 外围高的阶梯——本面板中位见收益率节；CCR-vs-其余稳固、RCR-vs-OCR 在本面板内薄）；
段内差异在本样本量下未检出——别信段内『高收益率盘』话术；
...
学校计数 = 30 所热门（超额认购）小学名单内 1km。样本围绕 9 个锚点盘聚集（稳健性检验按 14 簇做逐簇剔除：
9 个锚点 + 5 个独立单点项目），非全岛随机抽样；分段以 app Region 字段（Tier-1）为准。</p>
...
星号不可作卖点；改以<b>逐锚剔除的方向稳定性</b>为准：{loo_summary}。
读数只到『方向与相对强弱』：楼龄（新）≈ 学校 1km 最强，其余弱而方向稳定。
{esc(cs.get('segment_note', ''))}；量级不可外推全岛。</div>

<h2>收益率结构 <span class='en'>Yield structure</span></h2>
<p>本面板段位收益率（中位与区间，n 见括号）：{yield_bands}。
读法：<b>CCR 低收益率 vs 其余段的阶梯稳固；RCR-vs-OCR 差在本面板内薄</b>（市场风格化口径 CCR ~2-2.5% &lt;
OCR ~3-4% 在更广样本下成立，本面板标注为 market-stylized）。
...
<li>分段口径：以 app Region 字段（Tier-1）为准，邮政区启发式仅兜底（曾错标 3 行，评审后修正）；本面板 CCR 以 D10/D11 为主（7/10 行）+ D2 Spottiswoode/Tanjong Pagar 侧 3 行；圣淘沙与 D9 乌节核心未含</li>
...
def build_landed() -> str:
    ...
    # Frankel bands computed FROM the panel (a hand-typed band once dropped an
    # in-window $1,082 print — caught by review; never hand-type window bands)
    pl = load(os.path.join(F, "panel_landed_enriched.json"))
    fr = next(s for s in pl["streets"] if s["street"] == "Frankel Avenue")
    fr_prints = [(int(r["pp_date"][-4:]), r["pp_psf"]) for r in fr["pp_panel"]
                 if r.get("pp_psf") and r.get("pp_date")]
    early = [p for y, p in fr_prints if y <= 2006]
    recent = [p for y, p in fr_prints if y >= 2018]
    frankel_bands = (f"2000–2006 年印 ${min(early)}–{max(early)} psf → "
                     f"2018–2026 年印 ${min(recent):,}–{max(recent):,} psf（全部在册 PP 印，无剔除）")
    fr_stat = next(s for s in streets if s["street"] == "Frankel Avenue")
    fr_pairs, fr_pts = fr_stat["n_pairs"], fr_stat["n_points"]
    st_rows = ""
    for s in streets:
        if s["street"].startswith("Nearby ("):
            continue  # area-scope rows duplicate the street rows' transactions
        cg = s.get("cagr_median")
        band = s.get("street_band") or {}
        if band.get("low_psf"):
            band_txt = f"{band['low_psf']:,}–{band['high_psf']:,}"
        elif s.get("pp_psf_span"):
            lo, hi = s["pp_psf_span"]
            band_txt = f"{lo:,}–{hi:,}（PP 面全窗）"
        else:
            band_txt = "n/a（无成交带）"
        st_rows += (f"<tr><td class='l'>{esc(s['street'])}</td><td>{s['n_points']}</td>"
                    f"<td>{s['n_pairs']}</td>"
                    + (f"<td>约{cg * 100:+.0f}%/yr（指示性）</td>" if cg else "<td>n/a</td>")
                    + f"<td>{band_txt}</td>"
                    f"<td>{(s.get('nearest_mrt') or {}).get('km', 'n/a')}</td>"
...
重建状态价差巨大（Rosyth 同街同月实测 <b>+39%</b>，重建 detached vs 原状 semi-D 的混合价差——详见街道证据节）、
999y/FH 与 99y 是不同资产；
...
<div class='box'>Frankel Avenue（D15 东海岸 FH 带）：{frankel_bands}。
跨址同型配对（{fr_pairs} 对/{fr_pts} 址）中位约 <b>+6~7%/年</b>（一位有效数字；配对共享端点，有效样本远小于 {fr_pairs}）。
辩证团修正读法：①跨址对≠重复交易——含地块形态/翻建状态异质性（见下方 Rosyth 混合价差注），
...
<p class='note'>同街同月价差 +39%（9 Alnwick 重建后 <b>detached</b> vs 15 Alnwick 原状 <b>semi-D</b>，面积相当）——
注意这是『重建状态 × 户型形态』的<b>混合价差</b>（干净的同型重建对是数据缺口），但量级足以说明
重建经济学是 landed 最大的单一价差杠杆；『买 Serangoon Gardens = Rosyth 1km』被 OneMap 实测证伪。</p></div>
</div>
...
<li>β 调节：重建周期（原状→重建的一次性跃迁，Rosyth 混合价差 +39% 级——含户型形态成分）、GCBA 规划边界（政策稀缺）、遗产/供给刚性（landed 存量近零增长）</li>
...
<h2>风险与局限（完整版） <span class='en'>Risks & limitations — the full list</span></h2>
<ul>
<li>流动性：单街年成交常为个位数——报价区间必须按宽读，退出周期以季度计；指数点位不是压力期可成交价</li>
<li>样本与外推边界：街道证据集中于 3 个研究区（Nanyang/Rosyth/Frankel）——非全岛代表；跨址对共享端点，无有效标准误</li>
<li>政策制度断裂：ABSD 60%/SSD 4 年（2025-07 起）为史上最严组合——51 年历史 β/α 生成于更温和制度下；
landed 公民限购是额外的需求侧刚性（双向）</li>
<li>摩擦门槛：BSD 高价段 ~5-6% + 中介/法务 ~2-3% + SSD 4 年 16/12/8/4——毛 α 低于全程摩擦不是 α；重建套利需再扣建安（2021 后 +30-40%）与 2-3 年 carry</li>
<li>指数平滑/幸存者：官方 landed 序列平滑低估波动；app 数据只含在售可交易盘</li>
<li>利率制度：全部近年样本处于 ~1.4% 按揭时代——4% 情景下持有成本结构性变化</li>
<li>1996-98 教训：landed 并非永远防御——供给/杠杆共振时跌得更深（−48%）</li>
<li>混合口径提示：+39% 重建价差为『重建 × 户型形态』混合；街道 CAGR 含质量漂移（扣后 ~4.5-5.5%）</li></ul>
