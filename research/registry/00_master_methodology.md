...
| Landed | URA landed caveats (bundle: land+building) | walk-forward + measured NOISE FLOOR + case tier + REGIME SIGN PANEL | **SHIPPED (L0-L4, L2b): `landed-valuation`, engine LV1 = LC2 lease-matched street grid + fitted size curve → LA1 pooled fallback → conformal; time adjustment = published PPI + OBSERVED local-trend bridge (EXP-0017, per-comp anchor, never extrapolates). 9.05% median APE / 100% cover / 78.9% band. GL4 PASS 8.05.** Honest limits: ~6-8%/print bundle noise floor; unbiased in stable markets, residual ~4% low when accelerating (L2b closed ~5pp of ~16pp; "fixed" bar NOT met → disclosed. **EXP-0018 then closed the escape hatch: Investment Suite is NOT fresher than the URA API — same caveats, same lag — so there is no data lever either; the residual is a property of caveat publication and ships as disclosure, permanently**); ≥8k sqft case-tier |
